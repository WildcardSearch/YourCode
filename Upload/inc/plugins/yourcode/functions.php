...
namespace YourCode
{
	 ...
	 */
	function get_all($start = 0, $limit = '')
	{
		global $db;
		...
		if($db->num_rows($query) > 0)
		{
			...
			{
				$return_array[$this_data['id']] = new Simple($this_data);
			}
		}
		return $return_array;
	}
	 ...
	 */
	function build_url($options, $url = '')
	{
		if(!$url)
		{
			$url = YOURCODE_URL;
	 ...
	 */
	function build_link($url, $caption = "", $options = "")
	{
